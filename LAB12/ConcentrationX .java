...
import objectdraw.*;
import java.awt.*;
public class ConcentrationX extends WindowController {

    private static final int FLIPSIZE = 80;
    ...
    public void randomBackground(){
        randNum = new RandomIntGenerator(0, 5);
        if(randNum.nextValue() == 0)
            image = getImage("mandrill.jpg");
        else if(randNum.nextValue() == 1)
            image = getImage("horse.png");
        else if(randNum.nextValue() == 2)
            image = getImage("cat.jpg");
        else if(randNum.nextValue() == 3)
            image = getImage("monkey.jpg");
        else if(randNum.nextValue() == 4)
            image = getImage("os.jpg");
        else
            image = getImage("animal.jpg");
    ...
    public void onMouseClick(Location pt) {
        ...
        // check each ColorRect on board to see which was clicked
            ...
            for (col = 0; col < ROWSIZE; col++) {
               if (board[row][col].contains(pt) && clicked == false && board[row][col].isHidden() == false) {
                   ...
                   timer = new Timer(ROWSIZE, FLIPSIZE, canvas);
                    ...
                    //border[row][col].removeFromCanvas();
                    //border[selectedRow][selectedCol].removeFromCanvas();
