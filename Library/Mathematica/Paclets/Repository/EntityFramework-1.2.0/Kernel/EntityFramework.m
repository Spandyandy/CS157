...
$EntityFormatSpecialCaseTypes = {"Earthquake"};
...
EntityNameCacheFetch[Entity[type_, name_]] /; MemberQ[$EntityFormatSpecialCaseTypes, type] := specialCaseFormatting[type,name]
...
specialCaseFormatting[other_, name_] := name
...
ReleaseMWAComputeHold[args___] := ReleaseHold[ReplaceAll[args, 
	{Internal`MWASymbols`MWAHold[e_] :> With[{res = e}, res /; True], 
    remote_Internal`MWASymbols`MWARemoteObject :> fetchRemoteObject[remote]
    }]]

SetAttributes[Internal`MWASymbols`MWAData, HoldAll];(*see 268678*)
SetAttributes[Internal`MWASymbols`MWARemoteObject, HoldAll];

fetchRemoteObject[Internal`MWASymbols`MWARemoteObject[url_String, "ImportAction" -> fun_]] := 
 Module[{res}, res = Quiet[Check[fun[url], $Failed]];
  If[res === $Failed, Message[EntityValue::conopen, EntityValue]; 
   Missing["RetrievalFailure"], res]]
